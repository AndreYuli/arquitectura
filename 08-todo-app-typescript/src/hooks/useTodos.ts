...
import { getFilterStrategy } from '../strategies/filters'
...
const reducer = (state: State, action: Action): State => {
  if (action.type === 'INIT_TODOS') {
    ...
    TodoObserver.notify(todos) // Notificar a los observadores
    return {
      ...state,
      sync: false,
      todos
  ...
  if (action.type === 'REMOVE') {
    ...
    TodoObserver.notify(updatedTodos) // Notificar a los observadores
    return {
      ...state,
      sync: true,
      todos: updatedTodos
  ...
  if (action.type === 'SAVE') {
    ...
    const newTodo = TodoFactory.create(title)
...
export const useTodos = (): {
  ...
  const filteredTodos = getFilterStrategy(filterSelected).filter(todos)
